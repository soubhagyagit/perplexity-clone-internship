...
class Sandbox {
    ...
    async _runWs(command, options = {}) {
        ...
        const execCommandId = commandId ?? (0, uuid_js_1.uuid7)();
        let attempt = 0;
        ...
        while (true) {
            ...
            try {
                ...
                attempt++;
                if (attempt > command_handle_js_1.CommandHandle.MAX_AUTO_RECONNECTS) {
                    throw e;
                }
                const delay = Math.min(command_handle_js_1.CommandHandle.BACKOFF_BASE * 2 ** (attempt - 1), command_handle_js_1.CommandHandle.BACKOFF_MAX);
